...
using RestaurentManagementSystem;
...
namespace RestaurentManagementSystem
{
    public partial class CashierOrderForm : UserControl
    {
        ...
        {
            using (SqlConnection connect = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=|DataDirectory|\restManN2.mdf;Integrated Security=True;Connect Timeout=30"))
            {
                connect.Open();
        ...
        {
            if (cashierOrderForm_productID.SelectedIndex == -1 || cashierOrderForm_productID.SelectedItem == null)
            {
                cashierOrderForm_productName.Text = "";
                cashierOrderForm_price.Text = "";
                return;
            ...
            // .Trim() removes accidental spaces from the selection
            string selectedValue = cashierOrderForm_productID.SelectedItem.ToString().Trim();

            try
            {
                using (SqlConnection connect = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=E:\DBMS\RestaurentManagementSystem_Test\DB\restManN.mdf;Integrated Security=True;Connect Timeout=30;Encrypt=False"))
                {
                    connect.Open();

                    string selectData = "SELECT * FROM products WHERE prod_id = @id AND prod_status = @status";
                    ...
                    using (SqlCommand cmd = new SqlCommand(selectData, connect))
                    {
                        ...
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                // .Trim() ensures we don't display extra spaces
                                cashierOrderForm_productName.Text = reader["prod_name"].ToString().Trim();
                                cashierOrderForm_price.Text = reader["prod_price"].ToString().Trim();
                            }
                        }
                    }
                }
        ...
        */
        ...
        private void cashierOrderForm_orderAmount_TextChanged(object sender, EventArgs e)
        {

        }

        private void cashierOrderForm_orderAmount_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                try
                {
                    float getAmount = Convert.ToSingle(cashierOrderForm_orderAmount.Text);

                    float getChange = (getAmount - totalPrice);

                    if (getChange <= -1)
                    {
                        cashierOrderForm_orderAmount.Text = "";
                        cashierOrderForm_orderChange.Text = "";
                    }
                    else
                    {
                        cashierOrderForm_orderChange.Text = getChange.ToString();
                    }
        ...
        private void cashierOrderForm_orderPayBtn_Click(object sender, EventArgs e)
        {
            if (cashierOrderForm_orderAmount.Text == "" || cashierOrderForm_orderTable.Rows.Count < 0)
            {
                    ...
                    if (connect.State == ConnectionState.Closed)
                    {
                        try
                        {
                            ...
                            using (SqlCommand cmd = new SqlCommand(insertData, connect))
                            {
                                ...
                                cmd.Parameters.AddWithValue("@amount", cashierOrderForm_orderAmount.Text);
                                cmd.Parameters.AddWithValue("@change", cashierOrderForm_orderChange.Text);
        ...
        private void cashierOrderForm_orderReceiptBtn_Click(object sender, EventArgs e)
        {
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            string headerText = "Restaurent TS";
            ...
            e.Graphics.DrawString("Total Price: \t$" + totalPrice + "\nAmount: \t$"
                + cashierOrderForm_orderAmount.Text + "\n\t\t------------\nChange: \t$" + cashierOrderForm_orderChange.Text, labelFont, Brushes.Black, labelX, y);
